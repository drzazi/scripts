#!/usr/bin/env bash
# https://github.com/ToyoDAdoubiBackup/doubi/blob/master/socat.sh
# 修改自ToyoDAdoubiBackup/doubi，但支持ipv6
...
#设置 转发类型
    ...
    if [[ ${socattype_num} = "1" ]]; then
        ...
        socattype="TCP6"
    elif [[ ${socattype_num} = "5" ]]; then
        socattype="UDP6"
    elif [[ ${socattype_num} = "6" ]]; then
        socattype="TCP6+UDP6"
    else
        socattype="TCP+UDP"
    fi
...
startSocat(){
    if [[ ${socattype} = "TCP" ]]; then
    ...
    elif [[ ${socattype} = "TCP6" ]]; then
        runSocat "TCP6"
        sleep 2s
        PID=`ps -ef | grep "socat TCP6-LISTEN:${Socatport}" | grep -v grep | awk '{print $2}'`
        [[ -z $PID ]] && echo -e "${Error} Socat TCP6 启动失败 !" && exit 1
        addLocal "TCP6"
        ip6tables -I INPUT -p tcp --dport ${Socatport} -j ACCEPT
    elif [[ ${socattype} = "UDP6" ]]; then
        runSocat "UDP6"
        sleep 2s
        PID=`ps -ef | grep "socat UDP6-LISTEN:${Socatport}" | grep -v grep | awk '{print $2}'`
        [[ -z $PID ]] && echo -e "${Error} Socat UDP6 启动失败 !" && exit 1
        addLocal "UDP6"
        ip6tables -I INPUT -p udp --dport ${Socatport} -j ACCEPT
    elif [[ ${socattype} = "TCP6+UDP6" ]]; then
        runSocat "TCP6"
        runSocat "UDP6"
        sleep 2s
        PID=`ps -ef | grep "socat TCP6-LISTEN:${Socatport}" | grep -v grep | awk '{print $2}'`
        ...
        if [[ -z $PID ]]; then
            echo -e "${Error} Socat TCP6 启动失败 !" && exit 1
        else
            [[ -z $PID1 ]] && echo -e "${Error} Socat TCP6 启动成功，但 UDP6 启动失败 !"
            addLocal "TCP6"
            addLocal "UDP6"
            ip6tables -I INPUT -p tcp --dport ${Socatport} -j ACCEPT
            ip6tables -I INPUT -p udp --dport ${Socatport} -j ACCEPT
